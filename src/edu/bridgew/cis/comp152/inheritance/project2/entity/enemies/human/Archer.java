...
import edu.bridgew.cis.comp152.inheritance.project2.item.equipment.bow.Bow;
import edu.bridgew.cis.comp152.inheritance.project2.item.equipment.bow.LongBow;

public class Archer extends Human {

	// constructor
	public Archer(String name, double level) {
		super(name, "Archer" , 'M', 1 , "None",
				...
				Database.enemyBaseStats[Database.enemies.ARCHER.ordinal()][Database.statName.ATTACK.ordinal()], 
				Database.enemyBaseStats[Database.enemies.ARCHER.ordinal()][Database.statName.DEFENSE.ordinal()],
				Database.enemyBaseStats[Database.enemies.ARCHER.ordinal()][Database.statName.MAGICAL_DEFENSE.ordinal()],
				Database.enemyBaseStats[Database.enemies.ARCHER.ordinal()][Database.statName.SPEED.ordinal()],
				Database.enemyBaseStats[Database.enemies.ARCHER.ordinal()][Database.statName.HEALTH.ordinal()],
				Database.enemyBaseStats[Database.enemies.ARCHER.ordinal()][Database.statName.ACCURACY.ordinal()],
				Database.enemyBaseStats[Database.enemies.ARCHER.ordinal()][Database.statName.CRITICAL_DAMAGE.ordinal()],
				Database.enemyBaseStats[Database.enemies.ARCHER.ordinal()][Database.statName.CRITICAL_RATE.ordinal()],
				Database.enemySkills[Database.enemies.ARCHER.ordinal()],
				Main.iFact.getWeapon(Database.enemyWeapons[Database.enemies.ARCHER.ordinal()]));
	...
	// this method
	//  increases current HP and recovers lost stamina
	public void rest() {
		this.staminaBar += 20;
	...
	// this levels up and increases stats
	@Override
	public void levelUp() {
		this.level += 1;
		this.origHealth += Database.enemylevelUpStat[Database.enemies.ARCHER.ordinal()][Database.statName.HEALTH.ordinal()];
		this.currentHealth = this.origHealth;
		this.healthWithChanges = this.origHealth;

		this.origDef += Database.enemylevelUpStat[Database.enemies.ARCHER.ordinal()][Database.statName.DEFENSE.ordinal()];
		this.currentDef = this.origDef;

		this.origSpeed += Database.enemylevelUpStat[Database.enemies.ARCHER.ordinal()][Database.statName.SPEED.ordinal()];
		this.currentSpeed = this.origSpeed;

		this.origATK += Database.enemylevelUpStat[Database.enemies.ARCHER.ordinal()][Database.statName.ATTACK.ordinal()];
		this.currentAtk = this.origATK;

		this.origMDef += Database.enemylevelUpStat[Database.enemies.ARCHER.ordinal()][Database.statName.MAGICAL_DEFENSE.ordinal()];
		this.currentMDef = this.origMDef;

		this.origAccuracy += Database.enemylevelUpStat[Database.enemies.ARCHER.ordinal()][Database.statName.ACCURACY.ordinal()];
		this.currentAccuracy = this.origAccuracy;

		this.origCritDmg += Database.enemylevelUpStat[Database.enemies.ARCHER.ordinal()][Database.statName.CRITICAL_DAMAGE.ordinal()];
		this.currentCritDmg = this.origCritDmg;

		this.origCritRate += Database.enemylevelUpStat[Database.enemies.ARCHER.ordinal()][Database.statName.CRITICAL_RATE.ordinal()];
		this.currentCritRate = this.origCritRate;

		this.weight += 2;
	}

	// takes an array and calculates damage the archer takes, and then will decrease archer's HP
	@Override
	public boolean takeDmg(String[] dmgSpecs, double dmgCount) {
		...
		String elementDmg;
		double dmgTaken = 0;
		// specs for dmg decisions how to calculate
		damageType = dmgSpecs[0];
		elementDmg = dmgSpecs[1];
		// the folllowing if and else if statements
		// does damage to archer base on the damage type, then base on if the damage has a elemental attribute to it
	...
	// this is archer info with neccesary info for battle
	@Override
	...
	public void equipWeapon(Weapon weapon) {
		// if weapon is already equip, it will deequip
		if (this.weapon != null) {
			this.deEquipWeapon();
			weapon.useItem(this);
			this.weapon = weapon;
	...
	public boolean normalAttack(Entity entity, ArrayList<Enemy> enemies) {
		...
		if (attackChoice.equals("Weapon")) {
			...
			if (didHit == true) {
				if (didCrit == true) {
					dmgCount = this.currentAtk * 1.1 + this.currentCritDmg;
				} else if (didCrit == false) {
					dmgCount = (this.currentAtk * 1.1);
				}
				// if the dmg is negative or 0 it will set dmg to 1
				if (dmgCount <= 0) {
					dmgCount = 1;
