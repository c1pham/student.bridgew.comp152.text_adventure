...
public class ShireElf extends Elf {
	public ShireElf( String name, double level) {
		
		//  constructor
		super(name, "Shire Elf", 'M' , 1, "None",
				...
				Database.enemyBaseStats[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.ATTACK.ordinal()], 
				Database.enemyBaseStats[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.DEFENSE.ordinal()],
				Database.enemyBaseStats[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.MAGICAL_DEFENSE.ordinal()],
				Database.enemyBaseStats[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.SPEED.ordinal()],
				Database.enemyBaseStats[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.HEALTH.ordinal()],
				Database.enemyBaseStats[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.ACCURACY.ordinal()],
				Database.enemyBaseStats[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.CRITICAL_DAMAGE.ordinal()],
				Database.enemyBaseStats[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.CRITICAL_RATE.ordinal()],
				Database.enemySkills[Database.enemies.SHIRE_ELF.ordinal()],
				Main.iFact.getWeapon(Database.enemyWeapons[Database.enemies.SHIRE_ELF.ordinal()]),
				Database.enemyBaseStats[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.SHIELD_OR_ENERGY_DMG.ordinal()]);
	...
	// levels up elf increases stats
	@Override
	public void levelUp() {
		this.level += 1;
		this.origHealth += Database.enemylevelUpStat[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.HEALTH.ordinal()];
		this.currentHealth = this.origHealth;
		this.healthWithChanges = this.origHealth;

		this.origDef += Database.enemylevelUpStat[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.DEFENSE.ordinal()];
		this.currentDef = this.origDef;

		this.origSpeed += Database.enemylevelUpStat[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.SPEED.ordinal()];
		this.currentSpeed = this.origSpeed;

		this.origATK += Database.enemylevelUpStat[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.ATTACK.ordinal()];
		this.currentAtk = this.origATK;

		this.origMDef += Database.enemylevelUpStat[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.MAGICAL_DEFENSE.ordinal()];
		this.currentMDef = this.origMDef;

		this.origAccuracy += Database.enemylevelUpStat[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.ACCURACY.ordinal()];
		this.currentAccuracy = this.origAccuracy;

		this.origCritDmg += Database.enemylevelUpStat[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.CRITICAL_DAMAGE.ordinal()];
		this.currentCritDmg = this.origCritDmg;

		this.origCritRate += Database.enemylevelUpStat[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.CRITICAL_RATE.ordinal()];
		this.currentCritRate = this.origCritRate;

		this.origElementalShield += Database.enemylevelUpStat[Database.enemies.SHIRE_ELF.ordinal()][Database.statName.SHIELD_OR_ENERGY_DMG.ordinal()];
		this.curElementalShield = this.origElementalShield;

		this.weight += 2;
	}

	// takes an array and calculates damage the elf takes, and then will decrease elf's HP
	@Override
	public boolean takeDmg(String[] dmgSpecs, double dmgCount) {
		// if someone is already dead print this
		...
		String elementDmg;
		double dmgTaken = 0;
		// specs for type of dmg and how much
		damageType = dmgSpecs[0];
		elementDmg = dmgSpecs[1];
		// if dmg is elental and elf has shield dmg is absorbed
		...
		// if entity has died return true
		if (this.currentHealth <= 0) {
	...
	public void equipWeapon(Weapon weapon) {
		// if you have weapon equip it unequips it first
		if (this.weapon != null) {
			this.deEquipWeapon();
			weapon.useItem(this);
			this.weapon = weapon;
	...
	public boolean normalAttack(Entity entity, ArrayList<Enemy> enemies) {
		...
		// use chance to determine whether you hit or kick, and also level
	...
	public boolean specialMove(Entity entity, ArrayList<Enemy> enemies) {
		// return false if no ability is executed
		boolean executeAbility = false;
		ArrayList<Ability> availableAbility = new ArrayList<Ability>();
		// go through current ability list  and checks if any skill can be used
		// all usable skills are moved to available ability array
		...
		// if any ability are available it will randomly one that is avaialble
	...
	public boolean useInventory(Entity entity, ArrayList<Enemy> enemies, boolean useHP) {
		// keeps track if item is used in battle
		boolean usedBattleItem = false;
		if (this.bag.getCurrentNumItems() == 0) {
			return false;
		} else {
			if (useHP == true) {
				// goes through inventory to find a hp potion
				// if one is found it will return true and use it
				// also delete it from bag
			...
			if (useHP == false) {
				// goes through inventory to find a stamina potion
				// if one is found it will return true and use it
				// also delete it from bag
				for (int i = 0 ; i < this.bag.getCurrentNumItems(); i++) {
					Item curItem = this.bag.getItemList().get(i);
	...
	public void combatAction(Entity entity, ArrayList<Enemy> enemies) {
		...
		// loops until a battle action is done
		while (didBattleAction == false) {
			if (tryHpPot == false) {
				// if a enemy is low on hp it will  try to use a hp potion
				for (Enemy e : enemies) {
				...
				// if enemy doesn't have enough stamina to use a skill it will try to use stamina portion
				for (Enemy e : enemies) {
					double lowestCostSkill = 200;
					// goes through list and keeps track of lowest costing ability
				...
				float chance2;
				String attackChoice = null;
				// chance is use to determine special or weapon attack
				chance = rand.nextFloat();
				...
				if (attackChoice.equals("Special")) {
					...
					// if no able to do special it will do normal attack
